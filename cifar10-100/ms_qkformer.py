...
class MS_QKformer(nn.Module):
    def __init__(self, in_channels=2, num_classes=11,
        ...
        stage1 = nn.ModuleList([Block_QKA(
            dim=embed_dims//4, mlp_ratio=mlp_ratios, num_heads = 8)
            for j in range(1)])
        
        
        patch_embed2 = Embed_1Max(in_channels=embed_dims // 4,
                                    embed_dims=embed_dims // 2)
        

        stage2 = nn.ModuleList([Block_QKA(
            dim=embed_dims//2, mlp_ratio=mlp_ratios, num_heads = 8)
            for j in range(1)])

        patch_embed3 = Embed_1Max(in_channels=embed_dims // 2,
